...
func disposeOneMsg(appInfo msql.Params, msg map[string]any) {
	...
	case `image`, `voice`, `video`, `file`:
		msg[`oss_url`] = GetUrlByMediaIdNew(appInfo, cast.ToString(msg[`media_id`]), msgtype)
	...
	case `miniprogram`:
		...
		msg[`thumb_url`] = GetUrlByMediaIdNew(appInfo, cast.ToString(msg[`thumb_media_id`]), `image`)
		msg[`page_path`] = msg[`pagepath`]
		delete(msg, `pagepath`)
	case `link`:
